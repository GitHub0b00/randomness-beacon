...
14. In the backend (MDB) folder, start the FastAPI webserver by the command: "uvicorn main:app --host 0.0.0.0 --port 8001 --reload". If the 3-tier system archetecture is used, the incomming call allowed origins might need to be changed. It is set in the "main.py" file in the MDB folder. It is set to origin = ["*"] currently. If the call is only from the internal subnet then it could be better that the allowed request origins are only the frontend server, as explained in the talk with Ang, Kai Hui.
